...
def test_unauthenticated_request_returns_401():
	...
	resp = requests.get(v1_url("sys/health"), allow_redirects=False)
...
def test_authenticated_health_check(session):
	...
	resp = session.get(v1_url("sys/health"))
...
def test_x_vault_token_auth(vault_token_session):
	...
	resp = vault_token_session.get(v1_url("sys/health"))
	assert resp.status_code == 200
	body = resp.json()
	assert "initialized" in body
	assert "sealed" in body
...
def test_x_vault_token_write_read_roundtrip(vault_token_session):
	...
	vault_token_session.delete(v1_url(f"secret/metadata/frappe/http-test/vault-token-auth"))
	write = vault_token_session.post(v1_url(path), json={"data": {"value": "vault-native"}})
	assert write.status_code == 200
	read = vault_token_session.get(v1_url(path))
...
def test_blocked_paths_return_403(session, blocked_path):
	...
	resp = session.get(v1_url(blocked_path))
...
def cleanup_test_secret(session):
	...
	session.delete(v1_url(_TEST_METADATA_PATH))
	yield
	session.delete(v1_url(_TEST_METADATA_PATH))
...
def test_write_secret(session):
	...
	resp = session.post(v1_url(_TEST_SECRET_PATH), json={"data": {"value": "s3cr3t-value"}})
	assert resp.status_code == 200
...
def test_read_secret(session):
	...
	session.post(v1_url(_TEST_SECRET_PATH), json={"data": {"value": "hello-vault"}})
	resp = session.get(v1_url(_TEST_SECRET_PATH))
	assert resp.status_code == 200
	assert resp.json()["data"]["data"]["value"] == "hello-vault"


def test_read_nonexistent_secret_returns_404(session):
	resp = session.get(v1_url("secret/data/frappe/http-test/does-not-exist/field"))
	assert resp.status_code == 404


def test_delete_secret(session):
	"""DELETE on the metadata path should make the secret unreadable."""
	session.post(v1_url(_TEST_SECRET_PATH), json={"data": {"value": "to-be-deleted"}})
	del_resp = session.delete(v1_url(_TEST_METADATA_PATH))
	assert del_resp.status_code in (200, 204)
	assert session.get(v1_url(_TEST_SECRET_PATH)).status_code == 404


def test_secret_update_increments_version(session):
	"""Writing to the same path twice must increment the KV v2 version number."""
	resp1 = session.post(v1_url(_TEST_SECRET_PATH), json={"data": {"value": "v1"}})
	resp2 = session.post(v1_url(_TEST_SECRET_PATH), json={"data": {"value": "v2"}})
	assert resp1.status_code == 200
	assert resp2.status_code == 200
	assert resp2.json()["data"]["version"] == resp1.json()["data"]["version"] + 1
	assert session.get(v1_url(_TEST_SECRET_PATH)).json()["data"]["data"]["value"] == "v2"
