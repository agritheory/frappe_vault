...
def create_test_data():
	"""Create comprehensive test data for vault secrets and test users."""
	# Create users with vault OFF so bcrypt hashes land in __Auth,
	# then migrate them to OpenBao so tests can verify both paths.
	frappe.conf["enable_vault_user_passwords"] = False
	create_test_users()
	frappe.db.commit()
...
def create_test_users():
	...
	for user_data in users:
		...
		update_password(user_data["email"], user_data["password"])


def create_vault_secrets():
	"""Create test secrets with folder hierarchy demonstrating path nesting and tagging.

	With autoname: "field:path", each Vault Secret's doc.name equals its path.
	...
	for secret_data in secrets:
		...
		# With autoname: "field:path", doc.name == path — check by name directly
		if frappe.db.exists("Vault Secret", path):
			continue
